...
static const struct smb_init_register smb5_init_seq[] = {
	...
	/*
	 * Configure charge termination / recharge / inhibit policy.
	 * All set to 0 (their reset defaults): termination enabled,
	 * auto-recharge enabled, charger inhibit disabled, charge enable
	 * controlled by hardware. Without this the PM8150b can sit in
	 * INHIBIT/TERMINATE and report full at a low state of charge.
	 */
	{ .addr = CHGR_CFG2,
...
static int __used smb_probe(struct platform_device *pdev)
{
...
static const struct of_device_id __used smb_match_id_table[] = {
...
static struct platform_driver __used qcom_spmi_smb = {
	.probe = smb_probe,
	.driver = {
...
static int __used qcom_spmi_smb_init(void)
{
	return platform_driver_register(&qcom_spmi_smb);
}
device_initcall(qcom_spmi_smb_init);

static void __exit qcom_spmi_smb_exit(void)
{
	platform_driver_unregister(&qcom_spmi_smb);
}
